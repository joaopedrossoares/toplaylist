export const Genres = [
    {label:"Blues", value: "Blues"},
    {label:"Classic Rock", value:   "Classic Rock"},
    {label:"Country", value:   "Country"},
    {label:"Dance", value:   "Dance"},
    {label:"Disco", value:   "Disco"},
    {label:"Funk", value:   "Funk"},
    {label:"Grunge", value:   "Grunge"},
    {label:"Hip-Hop", value:   "Hip-Hop"},
    {label:"Jazz", value:   "Jazz"},
    {label:"Metal", value:   "Metal"},
    {label:"New Age", value:   "New Age"},
    {label:"Oldies", value:   "Oldies"},
    {label:"Other", value:   "Other"},
    {label:"Pop", value:   "Pop"},
    {label:"R&B", value:   "R&B"},
    {label:"Rap", value:   "Rap"},
    {label:"Reggae", value:   "Reggae"},
    {label:"Rock", value:   "Rock"},
    {label:"Techno", value:   "Techno"},
    {label:"Industrial", value:   "Industrial"},
    {label:"Alternative", value:   "Alternative"},
    {label:"Ska", value:   "Ska"},
    {label:"Death Metal", value:   "Death Metal"},
    {label:"Pranks", value:   "Pranks"},
    {label:"Soundtrack", value:   "Soundtrack"},
    {label:"Euro-Techno", value:   "Euro-Techno"},
    {label:"Ambient", value:   "Ambient"},
    {label:"Trip-Hop", value:   "Trip-Hop"},
    {label:"Vocal", value:   "Vocal"},
    {label:"Jazz+Funk", value:   "Jazz+Funk"},
    {label:"Fusion", value:   "Fusion"},
    {label:"Trance", value:   "Trance"},
    {label:"Classical", value:   "Classical"},
    {label:"Instrumental", value:   "Instrumental"},
    {label:"Acid", value:   "Acid"},
    {label:"House", value:   "House"},
    {label:"Game", value:   "Game"},
    {label:"Sound Clip", value:   "Sound Clip"},
    {label:"Gospel", value:   "Gospel"},
    {label:"Noise", value:   "Noise"},
    {label:"Alternative Rock", value:   "Alternative Rock"},
    {label:"Bass", value:   "Bass"},
    {label:"Soul", value:   "Soul"},
    {label:"Punk", value:   "Punk"},
    {label:"Space", value:   "Space"},
    {label:"Meditative", value:   "Meditative"},
    {label:"Instrumental Pop", value:   "Instrumental Pop"},
    {label:"Instrumental Rock", value:   "Instrumental Rock"},
    {label:"Ethnic", value:   "Ethnic"},
    {label:"Gothic", value:   "Gothic"},
    {label:"Darkwave", value:   "Darkwave"},
    {label:"Techno-Industrial", value:   "Techno-Industrial"},
    {label:"Electronic", value:   "Electronic"},
    {label:"Pop-Folk", value:   "Pop-Folk"},
    {label:"Eurodance", value:   "Eurodance"},
    {label:"Dream", value:   "Dream"},
    {label:"Southern Rock", value:   "Southern Rock"},
    {label:"Comedy", value:   "Comedy"},
    {label:"Cult", value:   "Cult"},
    {label:"Gangsta", value:   "Gangsta"},
    {label:"Top 40", value:   "Top 40"},
    {label:"Christian Rap", value:   "Christian Rap"},
    {label:"Pop/Funk", value:   "Pop/Funk"},
    {label:"Jungle", value:   "Jungle"},
    {label:"Native US", value:   "Native US"},
    {label:"Cabaret", value:   "Cabaret"},
    {label:"New Wave", value:   "New Wave"},
    {label:"Psychadelic", value:   "Psychadelic"},
    {label:"Rave", value:   "Rave"},
    {label:"Showtunes", value:   "Showtunes"},
    {label:"Trailer", value:   "Trailer"},
    {label:"Lo-Fi", value:   "Lo-Fi"},
    {label:"Tribal", value:   "Tribal"},
    {label:"Acid Punk", value:   "Acid Punk"},
    {label:"Acid Jazz", value:   "Acid Jazz"},
    {label:"Polka", value:   "Polka"},
    {label:"Retro", value:   "Retro"},
    {label:"Musical", value:   "Musical"},
    {label:"Rock & Roll", value:   "Rock & Roll"},
    {label:"Hard Rock", value:   "Hard Rock"},
    {label:"Folk", value:   "Folk"},
    {label:"Folk-Rock", value:   "Folk-Rock"},
    {label:"National Folk", value:   "National Folk"},
    {label:"Swing", value:   "Swing"},
    {label:"Fast Fusion", value:   "Fast Fusion"},
    {label:"Bebob", value:   "Bebob"},
    {label:"Latin", value:   "Latin"},
    {label:"Revival", value:   "Revival"},
    {label:"Celtic", value:   "Celtic"},
    {label:"Bluegrass", value:   "Bluegrass"},
    {label:"Avantgarde", value:   "Avantgarde"},
    {label:"Gothic Rock", value:   "Gothic Rock"},
    {label:"Progressive Rock", value:   "Progressive Rock"},
    {label:"Psychedelic Rock", value:   "Psychedelic Rock"},
    {label:"Symphonic Rock", value:   "Symphonic Rock"},
    {label:"Slow Rock", value:   "Slow Rock"},
    {label:"Big Band", value:   "Big Band"},
    {label:"Chorus", value:   "Chorus"},
    {label:"Easy Listening", value:   "Easy Listening"},
    {label:"Acoustic", value:   "Acoustic"},
    {label:"Humour", value:   "Humour"},
    {label:"Speech", value:   "Speech"},
    {label:"Chanson", value:   "Chanson"},
    {label:"Opera", value:   "Opera"},
    {label:"Chamber Music", value:   "Chamber Music"},
    {label:"Sonata", value:   "Sonata"},
    {label:"Symphony", value:   "Symphony"},
    {label:"Booty Bass", value:   "Booty Bass"},
    {label:"Primus", value:   "Primus"},
    {label:"Porn Groove", value:   "Porn Groove"},
    {label:"Satire", value:   "Satire"},
    {label:"Slow Jam", value:   "Slow Jam"},
    {label:"Club", value:   "Club"},
    {label:"Tango", value:   "Tango"},
    {label:"Samba", value:   "Samba"},
    {label:"Folklore", value:   "Folklore"},
    {label:"Ballad", value:   "Ballad"},
    {label:"Power Ballad", value:   "Power Ballad"},
    {label:"Rhythmic Soul", value:   "Rhythmic Soul"},
    {label:"Freestyle", value:   "Freestyle"},
    {label:"Duet", value:   "Duet"},
    {label:"Punk Rock", value:   "Punk Rock"},
    {label:"Drum Solo", value:   "Drum Solo"},
    {label:"Acapella", value:   "Acapella"},
    {label:"Euro-House", value:   "Euro-House"},
    {label:"Dance Hall", value:   "Dance Hall"},
    {label:"Goa", value:   "Goa"},
    {label:"Drum & Bass", value:   "Drum & Bass"},
    {label:"Club - House", value:   "Club - House"},
    {label:"Hardcore", value:   "Hardcore"},
    {label:"Terror", value:   "Terror"},
    {label:"Indie", value:   "Indie"},
    {label:"BritPop", value:   "BritPop"},
    {label:"Negerpunk", value:   "Negerpunk"},
    {label:"Polsk Punk", value:   "Polsk Punk"},
    {label:"Beat", value:   "Beat"},
    {label:"Christian Gangsta Rap", value:   "Christian Gangsta Rap"},
    {label:"Heavy Metal", value:   "Heavy Metal"},
    {label:"Black Metal", value:   "Black Metal"},
    {label:"Crossover", value:   "Crossover"},
    {label:"Contemporary Christian", value:   "Contemporary Christian"},
    {label:"Christian Rock", value:   "Christian Rock"},
    {label:"Merengue", value:   "Merengue"},
    {label:"Salsa", value:   "Salsa"},
    {label:"Thrash Metal", value:   "Thrash Metal"},
    {label:"Anime", value:   "Anime"},
    {label:"JPop", value:   "JPop"},
    {label:"Synthpop",value:   "Synthpop}"}
];